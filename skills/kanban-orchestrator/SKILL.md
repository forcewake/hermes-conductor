...
description: Kanban orchestrator: decompose, route, verify. No executing.
version: 3.0.0
...
**Fan-out + fan-in (research → synthesize):** N research-style cards with no parents, one synthesis card with all of them as parents. For deep research that must produce durable files, use `references/research-swarm-artifact-pattern.md`: create `.hermes/research/<slug>/00-orchestration.md`, require one artifact file per worker, gate the synthesis task with parent dependencies, and subscribe the current chat/topic when appropriate.
...
**Reviewer remediation loop:** When a reviewer task blocks with actionable findings, do not summarize and stop. Read the latest reviewer comment, patch the implementation branch/worktree, run focused + full verification, commit the fix, add a concise `hermes kanban comment <review_task>` containing commit SHA and verification output, then `hermes kanban unblock <review_task>`. Repeat until the reviewer task reaches `done` and dependent PR/release tasks promote to `ready`. See `references/reviewer-remediation-loop.md`.
